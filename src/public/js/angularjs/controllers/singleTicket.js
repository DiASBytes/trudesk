...
define([
    ...
    return angular
        ...
        .controller('singleTicket', function (SessionService, SettingsService, $window, $rootScope, $scope, $http, $timeout, $q, $log) {
            ...
            $q.all([userHttpGet]).then(function () {
                $scope.selectedOwner = _.findWhere($scope.users, {
                    _id: $scope.ticketOwner
                })
            })
            ...
            $scope.updateTicketOwner = function () {
                var id = $('#__ticketId').html()
                if (id.length > 0) {
                    socket.ui.setTicketOwner(id, $scope.selectedOwner)
                }
            }
